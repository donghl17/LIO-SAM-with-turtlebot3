...
void removeNaN_dhl (const pcl::PointCloud<PointXYZIRT> &cloud_in,
...
public:
  ...
  removeNaN_dhl(*laserCloudIn, *laserCloudIn, indices);
// std::cout<<"dense?"<<laserCloudIn->is_dense<<endl;
...
// pcl::removeNaNFromPointCloud(*laserCloudIn, *indices);
        ...
        if (ringFlag == 0)
        {
            ...
            if (ringFlag == -1)
            {
            //     std::cout<<"change ring channel begin!"<<std::endl;
            //     for (int i = 0; i < (int)currentCloudMsg.fields.size(); ++i)
            // {
            //     currentCloudMsg.fields[i].name = "ring"  ;
            // }
            //     ringFlag = 1;
            //     std::cout<<"ring channel correct"<<std::endl;
                ROS_WARN("Point cloud ring channel not available, please configure your point cloud data!");
